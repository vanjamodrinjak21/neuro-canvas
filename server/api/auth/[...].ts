  ...
  callbacks: {
    ...
    async jwt({ token, user }) {
      if (process.env.AUTH_DEBUG === 'true') {
        console.log(`[Auth:jwt] called, hasUser=${!!user}, tokenId=${token?.id ? '<present>' : 'none'}`)
      }
    ...
    async session({ session, token }) {
      if (process.env.AUTH_DEBUG === 'true') {
        console.log(`[Auth:session] called, hasToken=${!!token}, expired=${(token as Record<string, unknown>)?.expired || false}`)
      }
